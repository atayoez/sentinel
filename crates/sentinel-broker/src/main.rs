...
/// Runtime dir + socket. Kept separate from the legacy `/run/sentinel/ts`
/// on-disk store so the two can coexist during the shim-rewire transition.
/// systemd provides the dir via `RuntimeDirectory=sentinel-broker` (0700).
const SOCK_DIR: &str = "/run/sentinel-broker";
const SOCK_PATH: &str = "/run/sentinel-broker/broker.sock";
